            ...
            <div v-for="(q, index) in questions" :key="q.id" class="question-card">
              ...
              <div v-if="q.options && Object.keys(q.options).length > 0" class="options-list">
                <div v-for="(text, key) in q.options" :key="key"
                     :class="['option-item', getOptionClass(key, q)]">
                  <span class="option-key">{{ key }}</span>
                  <span class="option-text">{{ text }}</span>
                  <span v-if="isCorrectAnswer(key, q)" class="option-mark correct-mark">✓ 正确答案</span>
                  <span v-if="isWrongAnswer(key, q)" class="option-mark wrong-mark">✗ 学生选择</span>
                </div>
              </div>
