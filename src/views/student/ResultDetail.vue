          ...
          <div v-for="(q, index) in questions" :key="q.id" class="question-item">
            ...
            <div v-if="q.options && Object.keys(q.options).length > 0" class="options-review">
              <div v-for="(option, key) in q.options" :key="key" class="option-review-item">
                <span :class="{
                  correct: q.type === 'multiple' ? String(q.correct_answer).split(',').map(s => s.trim()).includes(key) : q.correct_answer === key,
                  selected: q.type === 'multiple' ? (Array.isArray(q.user_answer) ? q.user_answer.includes(key) : String(q.user_answer).split(',').map(s => s.trim()).includes(key)) : q.user_answer === key
                }">
